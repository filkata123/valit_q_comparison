...
for ex_num in [10]:#range(17):
    ...
    latex_lines.append(rf"\caption{{Comparing performance of \ql{{}} to performance of model-free approaches (Problem {ex_num})}}")
    latex_lines.append(fr"\label{{tab:prob{ex_num}det}}")
    latex_lines.append(r"\resizebox{\textwidth}{!}{")
    latex_lines.append(r"\begin{tabular}{lccc}")
    latex_lines.append(r"\hline")
    latex_lines.append(
        f"Algorithm (Problem {ex_num}) & Runtime (mean $\pm$ std) & Convergence & Optimal Initial Cost2Go Time (mean $\pm$ std) \\\\"
    )
    latex_lines.append(r"\hline")
